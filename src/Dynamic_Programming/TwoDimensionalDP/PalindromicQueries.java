...
public class PalindromicQueries {
    ...
    private static void fill(char[] s, int n) {
        palindrome = new boolean[n][n];
        // filling the DP array diagonally
        for (int i = 0; i < n; i++) {
            int start = 0, end = i;
            while (end < n) {
                if (start == end) // string with single character
                    palindrome[start][end] = true;
                else if (end - start == 1) // string with 2 characters
                    palindrome[start][end] = s[start] == s[end];
                else
                    palindrome[start][end] = palindrome[start + 1][end - 1] && s[start] == s[end];
                start++;
                end++;
            }
        }
